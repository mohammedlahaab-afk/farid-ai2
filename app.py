...
from openai import OpenAI # سنستخدم نفس المكتبة لأنها متوافقة

# 1. ضع مفتاح Groq المجاني هنا
MY_FREE_KEY = "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" 

st.set_page_config(page_title="مشروع الدراسة", page_icon="🧠")
st.title("الكيان البشري الرقمي (نسخة الطالب المجانية)")

# 2. ربط الكود بخادم مجاني (Groq) بدلاً من OpenAI المدفوع
client = OpenAI(
    ...
    with st.chat_message("assistant"):
        try:
            response = client.chat.completions.create(
                model="llama3-8b-8192", # هذا الموديل مجاني وسريع جداً
            ...
            st.error(f"خطأ: تأكد من وضع المفتاح المجاني بشكل صحيح")
